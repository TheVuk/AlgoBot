        ...
        try:
            ...
            if (so.LBuy_Position == True):
                if (tickprice >= (so.LSL_Price + so.SL + so.TSL)):
                    ...
                    (abObj.three_min_pd_DF.loc[abObj.three_min_pd_DF.index[-1]]['ADX']
                     < abObj.three_min_pd_DF.loc[abObj.three_min_pd_DF.index[len(abObj.three_min_pd_DF) - 2]]['ADX']):
                    print(
                ...
                if len(so.titicks) > 0:
                    if (float(ticktime) - float(so.titicks[0][0])) >= so.TI:
                        ...
                        if len(so.avgs) == 3:
                            ...
                            if pv_delta >= so.DTH:
                                abObj.three_min_long_flags['SAPM'] = 1
                                so.TI_SAPM_LONG = ticktime + 240
                                # print("LFLAG - ",str(time.strftime("%D %H:%M:%S", time.localtime(int(ticktime)))), abObj.three_min_long_flags)
                            if abObj.three_min_long_flags['SAPM'] == 1 and abObj.three_min_long_flags['MAEMA'] == 1\
                                    and abObj.three_min_long_flags['ADX'] == 1 and abObj.three_min_long_flags['MACD'] == 1\
                                ...
                                abObj.three_min_long_flags['SAPM'] = 0

                            if pv_delta <= (so.DTH * -1):
                                abObj.three_min_shot_flags['SAPM'] = 1
                                so.TI_SAPM_SHORT = ticktime + 240
                                # print("SFLAG -",str(time.strftime("%D %H:%M:%S", time.localtime(int(ticktime)))), abObj.three_min_shot_flags)
                            if abObj.three_min_shot_flags['SAPM'] == 1 and abObj.three_min_shot_flags['MAEMA'] == 1\
                                    and abObj.three_min_shot_flags['ADX'] == 1 and abObj.three_min_shot_flags['MACD'] == 1\
                                ...
                                abObj.three_min_shot_flags['SAPM'] = 0
